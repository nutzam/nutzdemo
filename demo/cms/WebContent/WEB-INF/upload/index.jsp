...
<div class="content">
   <div class="left fl_left">
       ...
       <div class="zsxxlm"><span><a href="${ctx}/nav?navid=35&parent=18&deep=1"><img src="${ctx}/template/images/more.gif"/></a></span>学术小组</div>
        ...
        <div class="tzgg fl_left">
              <ul>
				...
				<c:forEach var="report" items="${reports}">
				  ...
				  <li>
				 	<span><fmt:formatDate value="${report.createDate}" pattern="yyyy-MM-dd"/></span>
   ...
   <div class="right fl_left">
       ...
       <div><img src="${ctx}/template/images/loginbottom.gif" width="262" height="5" /></div>
       <div class="xydtlmbg mar1">友情链接</div>
       <div class="xsxznr2">
       		<select>
         		<option value="0">国内相关学院</option>
         		<c:forEach var="link" items="${link1}">
         			<option value="${link.url}">${link.name}</option>
         		</c:forEach>
       		</select>
       		<select>
         		<option value="0">国内相关科研院所</option>
         		<c:forEach var="link" items="${link2}">
         			<option value="${link.url}">${link.name}</option>
         		</c:forEach>
       		</select>
       		<select>
        		<option value="0">国外相关大学、研究机构</option>
        		<c:forEach var="link" items="${link3}">
         			<option value="${link.url}">${link.name}</option>
         		</c:forEach>
       		</select>
       </div>
