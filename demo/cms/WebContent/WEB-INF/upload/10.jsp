                  ...
                  <dt>
                  	<c:choose>
              			<c:when test="${article.files ne null and fn:length(article.files) gt 0}">
              				<img src="${ctx}/article/${article.files[0].filePath}" width="226" height="164" alt="${article.title}" title="${article.title}"/>
              			</c:when>
              			<c:otherwise>
              				<img src="${ctx}/template/images/sample3.jpg" alt="${article.title}" title="${article.title}" width="226" height="164"/>
              			</c:otherwise>
              		</c:choose>	
                  </dt>
                  <dd>
                      ...
                      <p>
						${fn:substring(fht:delHTMLTag(article.content),0,150)}......
					  </p>
                      <b><a href="${ctx}/article/detail?nav=${navsec}&model=${model.id}&article=${article.id}">[查看详细] </a></b>
                  </dd>
                  <strong><img src="${ctx}/template/images/xyyrybpictop.png"/></strong>
              </dl>
            </c:forEach>
          </div>
      </div>
