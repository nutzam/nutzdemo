...
public class IndexPicAction extends BaseAction {
	...
	@Ok("Json")
	public String save(@Param("::pic.") IndexPic pic,Ioc ioc){
...
//		IndexPicDao dao = new  IndexPicDao(ioc);
		...
		if (maxPage > 1) {
			List<IndexPic> pics = basicDao.searchByPage(IndexPic.class, (currentPage + 1),SystemContext.PAGE_SIZE, "sortNumber");
			for (IndexPic p : pics) {
...
//		IndexPicDao dao = new  IndexPicDao(ioc);
		...
		StringBuffer sb = new StringBuffer("[");
		if (maxPage > 1) {
			List<IndexPic> pices = basicDao.searchByPage(IndexPic.class, (currentPage + 1),SystemContext.PAGE_SIZE, "sortNumber");
			int i=0;
...
//		IndexPicDao dao = new  IndexPicDao(ioc);
		...
		List<IndexPic>  result  = basicDao.searchByPage(IndexPic.class, currentPage,  SystemContext.PAGE_SIZE, "sortNumber");
		int count = basicDao.searchCount(IndexPic.class);
