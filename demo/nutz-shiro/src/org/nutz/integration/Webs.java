...
public class Webs {
	...
	public static Ioc ioc() {
		...
		synchronized (lock) {
			...
			if (servletContext != null) {
				String mainModule = servletContext
						.getInitParameter("nutz.modules");
				if (mainModule != null)
					try {
						makeIoc(mainModule);
						return ioc;
				...
				if (ioc != null)
					...
					Enumeration<String> names = servletContext
							.getAttributeNames();
	...
	public static void makeIoc(Class<? extends IocProvider> clazz, String[] args) {
		...
		if (ioc instanceof Ioc2) {
			((Ioc2) ioc).addValueProxyMaker(new ServletValueProxyMaker(config
					.getServletContext()));
		}
	}
}
