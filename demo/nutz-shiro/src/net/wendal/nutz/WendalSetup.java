...
public class WendalSetup implements Setup {
	...
	public void init(NutConfig config) {
		...
		if (!dao.exists(User.class)) {
			...
			createRole(dao, "user-superadmin", "用户超级管理员：拥有对用户的任意操作权限的角色",new Integer[]{5});
			createRole(dao, "user-admin", "用户管理员：拥有对用户的浏览、增加和编辑(不包括删除)权限的角色",new Integer[]{4});
			createRole(dao, "security-admin", "安全管理员：拥有对角色和权限的任意操作，对用户分配角色及对角色分配权限的权限",new Integer[]{7,8,9});
	...
	private void createRole(Dao dao,String name,String desc,Integer[] rPIDs)
	{
		...
		role.setName(name);
		List<Permission> list = dao.query(Permission.class, Cnd.where("id", "iN", rPIDs));
